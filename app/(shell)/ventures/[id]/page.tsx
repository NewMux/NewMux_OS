...
export default async function VentureDetailPage({
  ...
  const [rule, parties] = await Promise.all([
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <Badge tone="neutral" className="mb-4 capitalize">
        {venture.launchStatus.replace("_", " ")}
      </Badge>
      ...
      <Card className="mb-4">
        ...
        <p className="mb-2 text-sm text-secondary-foreground">
          {venture.brandDescription ?? "No description yet."}
        </p>
        {venture.websiteUrl ? (
          <a
      ...
      <Card>
        ...
        {rule ? (
          <div className="flex flex-col gap-1 text-sm text-secondary-foreground">
            {rule.splits.map((s, i) => (
              <p key={i}>
                {parties.find((p) => p.id === s.partyId)?.name ?? "Unknown"} —{" "}
                {(s.percentageBps / 100).toFixed(0)}%
              </p>
            ))}
          </div>
        ) : (
          <p className="text-sm text-muted-foreground">
            Not configured yet — ownership and split are intentionally left
            empty until finalized (PRD section 14).
          </p>
        )}
        <Link
          href="/settings"
