        ...
        {alerts.tasksDueTodayOrOverdue.length > 0 && (
          <div>
            ...
            {alerts.tasksDueTodayOrOverdue.map((t) => (
              <p
                key={t.id}
                className={t.overdue ? "text-danger" : "text-foreground"}
              >
                {t.title}{" "}
                {t.overdue && (
          ...
          <div>
            ...
            {alerts.dealFollowUps.map((r) => (
              ...
              >
                <span
                  className={
                    r.status === "overdue" ? "text-danger" : "text-warning"
                  }
                >
        ...
        {alerts.hostingAlerts.length > 0 && (
          <div>
            ...
            {alerts.hostingAlerts.map((h) => (
              <p
                key={h.subscription.id}
                className={h.overdue ? "text-danger" : "text-warning"}
              >
                {centsToDisplay(
                  h.subscription.amountCents,
                  h.subscription.currency,
                )}{" "}
                {h.subscription.item}
                {h.subscription.nextDueDate
                  ? ` — due ${new Date(h.subscription.nextDueDate).toLocaleDateString()}`
                  : ""}
                {h.overdue ? " (overdue)" : ""}
              </p>
            ))}
