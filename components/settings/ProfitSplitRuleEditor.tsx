...
import type { Party, DeductionType, ProfitSplitRule, Project, Venture } from "@/lib/data/types";
...
export function ProfitSplitRuleEditor({
  ...
  const scopes = useMemo(
    () => [
      ...projects.map((p) => ({ scopeType: "project" as const, scopeId: p.id, label: `Project: ${p.name}` })),
      ...ventures.map((v) => ({ scopeType: "venture" as const, scopeId: v.id, label: `Venture: ${v.name}` })),
    ],
    [projects, ventures],
  );

  const [scopeKey, setScopeKey] = useState(scopes[0] ? `${scopes[0].scopeType}:${scopes[0].scopeId}` : "");
  const selectedScope = scopes.find((s) => `${s.scopeType}:${s.scopeId}` === scopeKey);
  const existingRule = rules.find((r) => r.scopeType === selectedScope?.scopeType && r.scopeId === selectedScope?.scopeId);

  const [splits, setSplits] = useState<SplitRow[]>(
    existingRule?.splits.map((s) => ({ partyId: s.partyId, percentage: String(s.percentageBps / 100) })) ?? [],
  );
  const [deductions, setDeductions] = useState<DeductionRow[]>(
    existingRule?.deductions.map((d) => ({
      deductionTypeId: d.deductionTypeId,
      value: String(deductionTypes.find((t) => t.id === d.deductionTypeId)?.kind === "percentage" ? d.value / 100 : d.value / 1000),
    })) ?? [],
  );
  ...
  function loadScope(key: string) {
    ...
    const rule = rules.find((r) => r.scopeType === scope?.scopeType && r.scopeId === scope?.scopeId);
    setSplits(rule?.splits.map((s) => ({ partyId: s.partyId, percentage: String(s.percentageBps / 100) })) ?? []);
    setDeductions(
  ...
  async function handleSave() {
    ...
    const res = await fetch("/api/finance/profit-split-rules", {
      ...
      body: JSON.stringify({
        ...
        deductions: deductions.map((d) => {
          ...
          return { deductionTypeId: d.deductionTypeId, value: type?.kind === "percentage" ? Math.round(raw * 100) : Math.round(raw * 1000) };
        }),
      }),
    });
    setSaving(false);
    if (!res.ok) {
      ...
      setError(typeof body?.error === "string" ? body.error : "Could not save split rule.");
      return;
    }
    router.refresh();
  }
  ...
  return (
    <Card>
        ...
        {splits.map((row, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <Input
              ...
              onChange={(e) => setSplits(splits.map((r, idx) => (idx === i ? { ...r, percentage: e.target.value } : r)))}
            />
            <button
      ...
      <div className="mb-4 flex flex-col gap-2">
          ...
          return (
            <div key={i} className="flex items-center gap-2">
              ...
              <Input
                ...
                value={row.value}
                onChange={(e) => setDeductions(deductions.map((r, idx) => (idx === i ? { ...r, value: e.target.value } : r)))}
              />
              <button
        ...
        <Button variant="ghost" size="sm" onClick={() => setDeductions([...deductions, { deductionTypeId: "", value: "" }])}>
